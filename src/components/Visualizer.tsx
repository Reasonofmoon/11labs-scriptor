...
// Deterministic heights for idle bars to prevent hydration mismatches
const IDLE_HEIGHTS = [12, 20, 15, 24, 18, 14, 22, 16];
...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradients strictly outside requestAnimationFrame to avoid recreating them on every frame (reduces GC pressure)
    // Web Audio API gets frequency data into a Uint8Array, mapping to 0-255 values, so we can precalculate precisely 256 gradients
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Implement short-circuit break condition to skip off-screen rendering calculations
        if (x > canvas.width) break;

        const val = dataArray[i];
        const barHeight = (val / 255) * canvas.height;

        ctx.fillStyle = precalculatedGradients[val];
