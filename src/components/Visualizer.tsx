...
// Deterministic heights for idle state to prevent Next.js hydration mismatch
const IDLE_BAR_HEIGHTS = [12, 22, 10, 18, 14, 20, 16, 11];
...
export const Visualizer = React.memo<VisualizerProps>(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache gradients for all possible byte values (0-255) outside the hot render loop
    // Expected performance impact: Reduces garbage collection pressure and CPU time spent creating objects per frame
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Break early to prevent unnecessary off-screen drawing computations
