...
// Pre-calculate deterministic random heights to avoid hydration mismatches
const STATIC_HEIGHTS = [12.4, 18.2, 10.1, 23.5, 15.6, 9.8, 19.3, 14.7];
...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradients for all possible byte values (0-255)
    // This avoids creating 100+ objects per frame in the animation loop
    const gradientCache: CanvasGradient[] = [];
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle zero-height edge case to ensure finite coordinates
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Short-circuit processing for bars that are outside the canvas bounds
