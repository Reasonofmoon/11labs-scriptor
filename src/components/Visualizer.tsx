...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Bolt Performance: Pre-calculate the 256 possible gradients
    // Web Audio API getByteFrequencyData returns values strictly between 0 and 255.
    // By pre-calculating we eliminate createLinearGradient calls inside the rendering loop.
    const gradients = new Array(256);
    for (let i = 0; i <= 255; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle zero-height edge case
      const gradientHeight = Math.max(1, barHeight);
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Bolt Performance: Short-circuit break to avoid drawing off-screen bars
        if (x > canvas.width) break;

        const val = dataArray[i];
        const barHeight = (val / 255) * canvas.height;

        // O(1) gradient lookup instead of recreating
        ctx.fillStyle = gradients[val];
  ...
  if (!isPlaying) {
    // Bolt Performance: Use a deterministic array instead of Math.random() for initial hydration state
    // Prevents Next.js hydration mismatches
    const idleHeights = [16, 22, 12, 18, 14, 20, 10, 24];
