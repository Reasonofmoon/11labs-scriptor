...
const IDLE_BAR_HEIGHTS = [12, 20, 16, 24, 14, 18, 10, 22]; // Pre-defined heights to avoid Math.random() in render
...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradients to avoid creating them on every frame for every bin
    const gradientCache: CanvasGradient[] = [];
    for (let i = 0; i <= 255; i++) {
      const barHeight = Math.max(1, (i / 255) * canvas.height); // Protect against zero-height
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Short-circuit off-screen rendering
