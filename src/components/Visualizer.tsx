...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Bolt Optimization: Pre-calculate the 256 possible CanvasGradient objects
    // to avoid creating new objects on every frame for every bar,
    ...
    for (let i = 0; i < 256; i++) {
      ...
      // Handle zero-height edge case to avoid DOMException with infinite/NaN coordinates
      const safeBarHeight = barHeight || 1;
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Bolt Optimization: Short-circuit break condition to skip off-screen rendering
  ...
  if (!isPlaying) {
    // Bolt Optimization: Use a deterministic array of heights instead of Math.random()
    // during the render cycle to prevent react-hooks/purity errors and hydration mismatches.
