...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Pre-calculate all 256 possible gradients based on 0-255 Uint8Array values
    // to eliminate thousands of object allocations per frame and reduce GC pressure.
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = Math.max(1, (i / 255) * canvas.height); // Max 1 to avoid non-finite DOMException
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt: Early break to avoid rendering off-screen elements
        if (x > canvas.width) break;

        const value = dataArray[i];
        const barHeight = (value / 255) * canvas.height;

        // ⚡ Bolt: Use pre-calculated gradient from cache instead of creating new ones
  ...
  if (!isPlaying) {
    // ⚡ Bolt: Replace Math.random() with deterministic array to prevent Next.js hydration mismatch
