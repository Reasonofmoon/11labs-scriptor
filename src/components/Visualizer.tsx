...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  // Deterministic array to replace Math.random() and avoid Next.js hydration mismatches
  const [idleHeights] = React.useState([12, 20, 16, 22, 10, 24, 18, 14]);
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradient objects outside the rendering loop to reduce GC pressure
    // Web Audio frequency data is always returned in a Uint8Array (0-255)
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle zero height edge case to avoid non-finite coordinate DOMException
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Short-circuit to avoid calculating and rendering off-screen elements
