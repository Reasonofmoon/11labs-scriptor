...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate 256 gradients outside the render loop to prevent high Garbage Collection (GC) pressure.
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle zero-height edge cases with Math.max to ensure coordinates are finite
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Bolt Performance Optimization:
        // Short-circuit off-screen elements calculation
  ...
  if (!isPlaying) {
    // Bolt Performance Optimization:
    // Use deterministic arrays for visual idle states to avoid Next.js hydration mismatches
