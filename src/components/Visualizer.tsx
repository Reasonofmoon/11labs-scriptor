...
const VisualizerComponent: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate 256 gradient permutations to avoid doing this in the tight animation loop
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Bounds check optimization

        const dataValue = dataArray[i];
        const barHeight = (dataValue / 255) * canvas.height;

        ctx.fillStyle = gradientCache[dataValue];
