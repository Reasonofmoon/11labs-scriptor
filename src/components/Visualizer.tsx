...
// Pre-calculate deterministic bar heights for the idle state to avoid Next.js hydration mismatch
// and satisfy React purity rules.
const IDLE_BAR_HEIGHTS = [12, 20, 15, 22, 10, 18, 14, 24];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache gradients outside requestAnimationFrame for performance optimization.
    // Audio frequency data bounds to integers [0, 255] which allows perfect caching.
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle edge case where gradient size is essentially 0
      const h = barHeight > 0 ? barHeight : 1;
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Bounds checking: stop rendering if bars exceed canvas width
