...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Cache gradients outside the render loop
    // Uint8Array values range from 0 to 255, so we can pre-calculate all possible gradients
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle zero-height edge case to prevent non-finite coordinates
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt: Short-circuit break when off-screen
  ...
  if (!isPlaying) {
    // ⚡ Bolt: Use deterministic array instead of Math.random() to prevent hydration errors
    const IDLE_HEIGHTS = [12, 20, 16, 24, 18, 14, 22, 10];
    return (
