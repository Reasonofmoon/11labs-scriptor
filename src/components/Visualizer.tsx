...
const IDLE_BAR_HEIGHTS = [12, 20, 16, 24, 18, 14, 22, 10]; // Deterministic heights to prevent hydration mismatch
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Cache LinearGradient objects outside the render loop for all 256 possible byte values
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // ⚡ Bolt: Early break if rendering goes beyond canvas width
