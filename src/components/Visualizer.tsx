...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // This prevents allocating hundreds of objects per frame, vastly improving performance.
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // ⚡ Bolt: Fast-path to avoid rendering off-screen bars
