...
// Deterministic idle bar heights to prevent hydration mismatches and impure function calls during render
const IDLE_BAR_HEIGHTS = [14, 22, 16, 10, 18, 20, 12, 24];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Optimize: Pre-calculate 256 possible gradients outside the render loop
    // since dataArray values are strictly integers from 0 to 255.
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Optimize: Bounds check to prevent processing off-screen elements
