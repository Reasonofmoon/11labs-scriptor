...
// Pre-defined heights for idle state to prevent hydration mismatches and unnecessary re-renders
const IDLE_BAR_HEIGHTS = [12, 16, 14, 20, 15, 18, 13, 19];
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Optimization: Stop drawing if we've exceeded the canvas width.
        // The barWidth calculation often results in total width > canvas.width.
        // This saves ~66% of iterations and expensive gradient creations.
        if (x > canvas.width) break;

        const barHeight = (dataArray[i] / 255) * canvas.height;

        // Note: Creating gradients inside a loop is expensive, but necessary for the specific per-bar gradient effect here.
        // By breaking the loop early, we significantly reduce the number of gradients created.
