...
const IDLE_BAR_HEIGHTS = [12, 16, 20, 14, 18, 10, 22, 15];
...
export const Visualizer = React.memo<VisualizerProps>(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache gradients for all possible byte values (0-255)
    // Keyed by value. Since colors change with mode, this cache is recreated when mode changes (dependency array)
    const gradientCache = new Array<CanvasGradient | undefined>(256);
