...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate gradients based on 0-255 Uint8Array bounds
    // Avoids creating thousands of CanvasGradient objects per second, reducing GC pressure
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Short-circuit off-screen calculations
  ...
  if (!isPlaying) {
    // ⚡ Bolt Optimization: Use deterministic constant array to prevent React hydration mismatches
    const IDLE_HEIGHTS = [12, 16, 20, 14, 18, 10, 22, 15];
    return (
