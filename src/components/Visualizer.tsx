...
const IDLE_BAR_HEIGHTS = [12, 18, 10, 22, 14, 20, 16, 24];
...
export const Visualizer: React.FC<VisualizerProps> = memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradients for the 256 possible byte values to avoid object creation in render loop
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Optimization: Stop rendering when off-screen
