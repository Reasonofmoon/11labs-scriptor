...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate gradients outside the render loop
    // Web Audio API frequency data is 0-255, so we can pre-calculate all possible gradients
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle zero-height edge case to avoid DOMException: The coordinates provided are not finite.
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Short-circuit break condition
        if (x > canvas.width) break;

        const barHeight = (dataArray[i] / 255) * canvas.height;

        // ⚡ Bolt Optimization: Use pre-calculated gradient
  ...
  if (!isPlaying) {
    // ⚡ Bolt Optimization: Deterministic array for hydration
    const heights = [14, 21, 10, 18, 12, 24, 9, 16];
    return (
