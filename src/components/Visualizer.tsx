...
const IDLE_BAR_HEIGHTS = [12, 16, 14, 20, 18, 15, 22, 14];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradient cache to avoid recreating expensive objects 60 times a second
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Optimization: Stop processing off-screen elements
