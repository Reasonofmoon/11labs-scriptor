...
const IDLE_BAR_HEIGHTS = [12, 16, 10, 20, 14, 18, 11, 15];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate 256 gradients since getByteFrequencyData returns values from 0-255
    const gradientCache = new Array<CanvasGradient>(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle the case where barHeight is 0
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Bounds check to avoid processing off-screen bars
