...
// Deterministic idle bar heights to prevent hydration mismatch and react-hooks/purity errors
const IDLE_BAR_HEIGHTS = [12, 18, 14, 20, 10, 16, 12, 22];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate CanvasGradient objects for all 256 possible byte values
    // to avoid instantiating expensive objects inside the requestAnimationFrame loop
    const gradientCache: CanvasGradient[] = [];
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Provide a fallback coordinates if barHeight is 0 (gradient won't be visible anyway)
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        ctx.fillStyle = gradientCache[dataArray[i]];
