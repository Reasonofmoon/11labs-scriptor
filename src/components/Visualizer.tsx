...
// Deterministic values for idle state bars to avoid hydration mismatch and purity errors
const IDLE_BAR_HEIGHTS = [12, 22, 16, 24, 10, 18, 14, 20];

// Memoize to prevent unnecessary re-renders when parent state updates
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache gradients for all possible byte frequency values (0-255) to avoid
    // expensive createLinearGradient calls inside the animation loop
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Prevent unnecessary processing and drawing of off-screen bars
