...
const IDLE_BAR_HEIGHTS = [12, 18, 14, 20, 16, 22, 15, 19];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradients for all possible 256 byte values to avoid recreating them every frame
    // This removes massive GC pressure from the 60fps render loop
    ...
    const draw = () => {
      ...
      // Using rgba for the "trailing" or "motion blur" fade-out visual effect
