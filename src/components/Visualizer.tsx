...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate 256 possible gradients
    // Web Audio frequency data is always a Uint8Array (values 0-255).
    // Pre-calculating these avoids creating thousands of CanvasGradient objects per frame.
    const gradientCache: CanvasGradient[] = [];
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle edge case where barHeight is 0 which can cause coordinates to not be finite
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Short-circuit break
        // Stop calculating and drawing if we're past the canvas width
  ...
  if (!isPlaying) {
    // ⚡ Bolt Optimization: Deterministic constant array instead of Math.random()
