...
// Pre-defined static heights to replace impure Math.random() in render, preventing hydration mismatches.
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate all 256 possible CanvasGradient objects
    // since frequency data integers are strictly between 0 and 255.
    // This avoids creating linear gradients on every single animation frame,
    // dramatically reducing garbage collection overhead and cpu usage.
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Bounds check to stop processing off-screen bars
