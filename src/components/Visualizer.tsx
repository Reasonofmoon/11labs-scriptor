...
// Deterministic idle heights to prevent hydration mismatch and impure render
const IDLE_BAR_HEIGHTS = [12, 18, 14, 22, 16, 20, 15, 19];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache gradients for all 256 possible byte values to avoid creating thousands of gradients per frame
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Bounds check: don't process bars outside the visible canvas
