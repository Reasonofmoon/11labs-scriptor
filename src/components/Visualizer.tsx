...
// ⚡ Bolt Optimization: Use deterministic array for idle state to prevent hydration mismatches
// and avoid calling impure functions like Math.random() during render.
const IDLE_BAR_HEIGHTS = [12, 20, 15, 22, 18, 24, 16, 14];

// ⚡ Bolt Optimization: Wrap component in React.memo to prevent unnecessary re-renders
// since this component is frequently updated by parent state changes but only needs
// to re-render when its specific props change.
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Cache gradient objects outside the render loop
    // Creating CanvasGradients is expensive and we only need one per discrete height
    // in the typical 256-value byte range.
    const gradientCache = new Map<number, CanvasGradient>();

    const getGradient = (barHeight: number) => {
      // Round height to nearest integer to increase cache hits
      const key = Math.round(barHeight);
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Break early if we've drawn past the canvas width
        // Prevents processing off-screen elements in the frequency data array
        if (x > canvas.width) break;

        const barHeight = (dataArray[i] / 255) * canvas.height;

        ctx.fillStyle = getGradient(barHeight);
