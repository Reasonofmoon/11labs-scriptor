...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  // Deterministic heights for the idle state to avoid hydration mismatches
  const IDLE_HEIGHTS = [12, 18, 14, 22, 10, 16, 20, 15];
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate the gradients for all possible byte values (0-255).
    // The Web Audio API dataArray uses integers from 0-255, so we can cache these
    // outside the `requestAnimationFrame` loop to drastically reduce Garbage Collection
    // (GC) pressure and avoid creating 1000+ CanvasGradient objects per frame.
    const gradientCache = new Array<CanvasGradient>(256);
    for (let val = 0; val < 256; val++) {
      const height = (val / 255) * canvas.height;
      // Handle 0-height edge case for `createLinearGradient` to ensure coordinates are finite
      const gradient = ctx.createLinearGradient(0, canvas.height - Math.max(1, height), 0, canvas.height);
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Short-circuit loop if rendering off-screen
