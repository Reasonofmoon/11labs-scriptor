...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate 256 CanvasGradient objects to avoid high object allocation per frame
    const gradients = new Array(256);
    for (let i = 0; i < 256; i++) {
      const h = Math.max(1, (i / 255) * canvas.height); // Handle zero-height edge case
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // ⚡ Bolt Optimization: Short-circuit off-screen elements
  ...
  if (!isPlaying) {
    return (
      <div className="flex items-center gap-1.5 px-4 py-2 bg-slate-800/50 rounded-xl border border-slate-700">
        {/* ⚡ Bolt Optimization: Replaced Math.random() with deterministic array to fix Next.js hydration mismatch */}
        {[12, 18, 14, 22, 10, 20, 16, 24].map((height, i) => (
          <div
            key={i}
