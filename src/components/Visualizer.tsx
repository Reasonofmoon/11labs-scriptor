...
// Deterministic idle heights to fix hydration mismatch and avoid impure function during render
const IDLE_BAR_HEIGHTS = [12, 20, 16, 24, 18, 14, 22, 10];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradient cache to avoid recreating objects inside render loop
    // Uint8Array values are strictly between 0 and 255
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        if (value > 0) {
          ...
          ctx.fillStyle = gradientCache[value] || primaryColor;
