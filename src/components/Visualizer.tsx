...
// Deterministic heights for the idle state to prevent hydration mismatches
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate and cache CanvasGradient objects for all 256 possible byte values.
    // Web Audio API getByteFrequencyData populates a Uint8Array with values 0-255.
    // This avoids creating 60+ new CanvasGradient objects *per frame* inside the render loop,
    // which prevents significant memory allocation and stops GC pauses during animation.
    const gradientCache = new Array(256);
    for (let val = 0; val < 256; val++) {
      const barHeight = (val / 255) * canvas.height;
      // We only need gradients if barHeight > 0 to avoid createLinearGradient errors with 0 height differences,
      // though typically browser handles it gracefully, it's safer.
    ...
    const draw = () => {
      ...
      // We maintain the slight trailing effect by not fully clearing
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        x += barWidth;
        if (x > canvas.width) break; // Optimization: stop drawing if we're off canvas
      }
    };
