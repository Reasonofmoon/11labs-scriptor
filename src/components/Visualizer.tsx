...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradients for all possible frequency values (0-255) to reduce GC pressure
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Optimization: skip drawing off-screen elements
  ...
  if (!isPlaying) {
    // Static heights to prevent hydration mismatch from Math.random()
    const idleHeights = [12, 18, 14, 22, 16, 10, 20, 15];
    return (
