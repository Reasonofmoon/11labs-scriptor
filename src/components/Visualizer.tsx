...
// Pre-defined random heights to ensure React component purity and avoid hydration mismatch
const IDLE_BAR_HEIGHTS = [12.4, 18.2, 10.5, 21.1, 15.3, 19.8, 11.2, 16.7];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache linear gradients for each possible frequency value (0-255)
    // This avoids creating expensive CanvasGradient objects on every frame loop
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Optimization: stop drawing if we're off the canvas
        ...
        if (barHeight > 0) {
          ctx.fillStyle = gradientCache[val] || primaryColor;
