...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Bolt Optimization: Pre-calculate 256 gradients (since Uint8Array values are 0-255)
    // This avoids recreating CanvasGradient objects 60 times a second per frequency bin,
    // drastically reducing Garbage Collection (GC) pressure.
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle zero-height edge case to avoid DOMException: coordinates are not finite
      const validHeight = Math.max(1, barHeight);

      const gradient = ctx.createLinearGradient(0, canvas.height - validHeight, 0, canvas.height);
  ...
  if (!isPlaying) {
    // Bolt Optimization: Replace Math.random() with deterministic array
    // to avoid Next.js hydration mismatches between server and client.
    const IDLE_HEIGHTS = [14, 22, 11, 20, 16, 24, 10, 18];
