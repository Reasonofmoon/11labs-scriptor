...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // This avoids creating 100+ gradients per frame during requestAnimationFrame
    const gradientCache = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle edge case where barHeight is 0, which can cause finite coordinate errors
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Short-circuit: skip off-screen calculations
  ...
  if (!isPlaying) {
    // Static heights array to prevent Next.js hydration mismatches from Math.random()
    const IDLE_HEIGHTS = [12, 20, 16, 24, 14, 18, 10, 22];
