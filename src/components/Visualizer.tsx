...
// Deterministic heights to avoid Next.js hydration mismatches
const IDLE_BAR_HEIGHTS = [12, 20, 15, 24, 10, 18, 14, 22];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache gradients for all possible byte frequency values (0-255)
    // This avoids excessive memory allocation inside requestAnimationFrame
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Prevent rendering off-screen elements
