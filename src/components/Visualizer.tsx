...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradient cache to avoid creating them in requestAnimationFrame
    // Since Web Audio freq data is strictly 0-255, we pre-create all possible gradients
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Optimization: Stop rendering when bars exceed canvas width
  ...
  if (!isPlaying) {
    const defaultHeights = [14, 21, 10, 18, 24, 12, 19, 15]; // Deterministic heights
    return (
      <div className="flex items-center gap-1.5 px-4 py-2 bg-slate-800/50 rounded-xl border border-slate-700">
        {[...Array(8)].map((_, i) => (
          <div
            ...
            style={{ height: `${defaultHeights[i]}px` }}
          />
        ))}
      </div>
    );
