...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Pre-calculate the 256 possible CanvasGradients (0-255 bounds from Uint8Array)
    // This avoids creating thousands of new gradient objects per second in the render loop,
    // drastically reducing Garbage Collection (GC) pressure and CPU overhead.
    const gradientCache = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle zero height edge case to avoid infinite coordinates error
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt: Short-circuit loop if x is beyond the canvas bounds
        if (x > canvas.width) break;

        const val = dataArray[i];
        const barHeight = (val / 255) * canvas.height;

        // ⚡ Bolt: Use pre-calculated gradient from cache
  ...
  if (!isPlaying) {
    // ⚡ Bolt: Use a static array instead of Math.random() during render to fix React Hydration errors
    const idleHeights = [14, 22, 16, 20, 12, 18, 24, 10];
      ...
      <div className="flex items-center gap-1.5 px-4 py-2 bg-slate-800/50 rounded-xl border border-slate-700">
        {[...Array(8)].map((_, i) => (
          <div
            ...
            style={{ height: `${idleHeights[i % idleHeights.length]}px` }}
          />
        ))}
      </div>
    );
