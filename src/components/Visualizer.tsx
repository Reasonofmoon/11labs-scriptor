...
const IDLE_BAR_HEIGHTS = [12, 20, 15, 22, 10, 18, 14, 24]; // Pre-calculated deterministic heights
...
export const Visualizer = React.memo<VisualizerProps>(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate CanvasGradient objects for all 256 possible byte values to optimize rendering performance
    // 💡 Performance: Eliminates object instantiation in the animation loop
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // 💡 Performance: Stop rendering if bars exceed canvas width
