...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache gradients for all possible frequency values (0-255)
    // This avoids instantiating CanvasGradient objects per frame per bin, avoiding GC pauses
    const gradientCache = new Array<CanvasGradient>(256);

    // Pre-calculate the gradient cache
    for (let i = 0; i <= 255; i++) {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Break early if we've rendered past the visible canvas
