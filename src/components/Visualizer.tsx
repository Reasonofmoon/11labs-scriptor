...
// ⚡ Bolt: Use deterministic idle bar heights to fix React purity lint error
const IDLE_BAR_HEIGHTS = [12, 18, 15, 22, 10, 16, 20, 14];

// ⚡ Bolt: Wrap in React.memo to prevent unnecessary re-renders
export const Visualizer = React.memo(({ isPlaying, mode, analyser }: VisualizerProps) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Pre-calculate CanvasGradients outside the requestAnimationFrame loop
    // Since getByteFrequencyData returns 0-255, we have 256 possible bar heights.
    const gradientCache = new Array(256).fill(null);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // We only need gradients if height is > 0
      if (barHeight > 0) {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt: Bounds check to stop processing off-screen elements
        ...
        if (dataValue > 0) {
          ...
          ctx.fillStyle = gradientCache[dataValue] || primaryColor;
