...
// Deterministic idle state heights to avoid hydration mismatches
// and React strict mode errors with Math.random()
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache linear gradients by height to avoid recreating them
    // inside the hot drawing loop (optimization)
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Bounds checking: prevent processing off-screen bars
        if (x > canvas.width) break;

        const barHeight = (dataArray[i] / 255) * canvas.height;

        // Cache gradient objects based on integer height
        const intHeight = Math.floor(barHeight);
        let gradient = gradientCache.get(intHeight);

        if (!gradient) {
            gradient = ctx.createLinearGradient(0, canvas.height - intHeight, 0, canvas.height);
            gradient.addColorStop(0, primaryColor);
            gradient.addColorStop(1, secondaryColor);
            gradientCache.set(intHeight, gradient);
        }
