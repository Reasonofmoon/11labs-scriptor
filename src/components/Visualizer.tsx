...
const IDLE_BAR_HEIGHTS = [12, 16, 14, 20, 10, 18, 15, 12];
...
const VisualizerComponent: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate and cache LinearGradient objects for all 256 possible Uint8 values
    // This avoids creating thousands of gradient objects per second during rendering
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle edge case where barHeight is 0 (createLinearGradient requires distinct y values if x values are same)
