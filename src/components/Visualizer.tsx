...
// Deterministic heights for idle state to prevent Next.js hydration mismatches
// instead of using Math.random() in render loop.
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache 256 gradients based on possible Uint8Array values to prevent expensive object creation per frame
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // bounds check to prevent processing of off-screen elements
