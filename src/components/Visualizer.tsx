...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Bolt Optimization: Pre-calculate 256 gradient combinations based on Uint8Array bounds.
    // This avoids creating thousands of CanvasGradient objects per second in the render loop.
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Stop rendering if we go past canvas boundary
  ...
  // Static array to avoid Next.js hydration mismatch on Math.random()
  const IDLE_BAR_HEIGHTS = [12, 18, 14, 20, 16, 22, 10, 24];
