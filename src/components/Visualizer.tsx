...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  // Deterministic heights for idle bars to prevent hydration errors from Math.random()
  const IDLE_BAR_HEIGHTS = [12, 22, 16, 24, 14, 20, 10, 18];
  ...
  useEffect(() => {
    ...
    // Web Audio frequency data returns integers strictly bounded from 0 to 255.
    // By pre-calculating the 256 possible CanvasGradients once, we avoid allocating
    // hundreds of gradient objects per frame inside requestAnimationFrame.
    // This dramatically reduces Garbage Collection (GC) pressure and avoids jank.
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Performance Optimization: Short-circuit rendering for off-screen elements
