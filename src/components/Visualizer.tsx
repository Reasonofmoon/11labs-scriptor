...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradients to avoid creating them on every frame
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle edge case where barHeight is 0 (can cause gradient creation errors in some browsers if start/end are identical)
      const gradientHeight = Math.max(0.1, barHeight);
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Optimization: Stop drawing if we've gone past the right edge of the canvas
