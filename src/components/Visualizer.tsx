...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradient objects outside render loop based on bounded 0-255 frequency data
    // This avoids recreating LinearGradient objects 60 times a second
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Stop drawing if we are off-screen
        if (x > canvas.width) break;

        const val = dataArray[i];
        if (val > 0 && gradientCache[val]) {
