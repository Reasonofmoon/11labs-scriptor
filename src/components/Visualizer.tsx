...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradients for all possible values (0-255) to avoid recreating them in the loop
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Short-circuit if we're drawing off-screen
  ...
  if (!isPlaying) {
    // Deterministic heights to avoid hydration mismatch from Math.random()
    const heights = [12, 18, 14, 22, 10, 20, 16, 24];
    return (
