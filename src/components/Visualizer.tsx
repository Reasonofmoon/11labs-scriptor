...
// Pre-calculated deterministic heights for the idle state to avoid hydration errors
// and prevent calculating random values on every render
...
const VisualizerComponent: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Web Audio API frequency data is 0-255. We cache the gradient for each possible value
    // outside the render loop to prevent expensive garbage collection and recreation
    // on every single frame (~60fps * ~100 bars = 6000 gradients/sec saved!)
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Early exit if we're drawing past the canvas
...
// ⚡ Bolt Optimization: Memoize the component to prevent re-renders when parent state
// (like input text or other non-audio related states) changes
