...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Bolt Optimization: Pre-calculate gradients for all possible 256 Web Audio API values (Uint8Array: 0-255)
    // This avoids creating 100+ CanvasGradient objects per frame (~6000 objects/sec), significantly reducing GC pressure.
    const gradientCache = new Array(256);
    for (let i = 0; i < 256; i++) {
      const height = (i / 255) * canvas.height;
      // Math.max(1) ensures finite coordinates to prevent DOMException on zero-height gradients
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Bolt Optimization: Short-circuit break if we're rendering off-screen
  ...
  if (!isPlaying) {
    // Bolt Optimization: Replace Math.random() with deterministic heights to prevent Next.js hydration mismatches
    const idleHeights = [12, 20, 16, 22, 14, 18, 10, 24];
    return (
