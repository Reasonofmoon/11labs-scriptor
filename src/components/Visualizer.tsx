...
// Pre-calculated deterministic heights for the idle state to prevent hydration mismatches and impure renders
const IDLE_BAR_HEIGHTS = [12.5, 18.2, 10.1, 22.4, 15.8, 9.3, 20.6, 14.7];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate CanvasGradients for all possible 256 byte values
    // This avoids creating expensive gradient objects inside the tight requestAnimationFrame loop
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Early exit if we're rendering past the canvas width
