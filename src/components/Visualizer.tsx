...
// Deterministic bar heights for idle state to prevent hydration mismatches and impure function errors
const IDLE_BAR_HEIGHTS = [12, 20, 15, 24, 18, 10, 22, 16];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Pre-calculate gradients outside the animation loop
    // Since frequency data is bounded 0-255, we can cache all 256 possible gradients
    // This turns expensive gradient instantiations into an O(1) array lookup per frame per bar
    const gradientCache = new Array(256);
    for (let val = 0; val < 256; val++) {
      const barHeight = (val / 255) * canvas.height;
      if (barHeight === 0) continue; // Skip creating gradient for 0 height
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt: Prevent processing off-screen bars
