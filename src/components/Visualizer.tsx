...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Cache gradients for all possible byte values (0-255) outside the animation loop
    // This eliminates allocating ~1000 CanvasGradient objects per frame (~60k objects/sec)
    // which previously caused significant GC pressure and dropped frames
    const gradientCache = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = Math.max(1, (i / 255) * canvas.height); // Handle 0 height edge case
  ...
  if (!isPlaying) {
    ...
    // Math.random() during render causes client/server mismatch errors
    const idleHeights = [12, 18, 14, 22, 16, 20, 10, 24];
    return (
