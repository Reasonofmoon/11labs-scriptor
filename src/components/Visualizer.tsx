...
const IDLE_BAR_HEIGHTS = [12, 18, 14, 22, 16, 20, 10, 15];
...
export const Visualizer = React.memo(({ isPlaying, mode, analyser }: VisualizerProps) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate gradients to avoid recreation on every frame
    const gradientCache = new Array<CanvasGradient>(256);
    for (let i = 0; i <= 255; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle the case where barHeight is exactly 0
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Stop drawing if we are out of bounds
