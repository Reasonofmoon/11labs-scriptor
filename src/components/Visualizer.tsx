...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate 256 canvas gradients outside the render loop
    // Web Audio API frequency data (Uint8Array) is bounded between 0-255.
    // Creating CanvasGradients on every frame inside requestAnimationFrame creates massive GC pressure.
    const gradientCache = new Array(256);
    for (let i = 0; i < 256; i++) {
      const height = (i / 255) * canvas.height;
      // Prevent DOMException for non-finite coordinates on 0 height
  ...
  if (!isPlaying) {
    ...
    // Prevents Next.js hydration mismatches between server and client
    const idleHeights = [12, 20, 15, 22, 10, 18, 14, 24];
    return (
