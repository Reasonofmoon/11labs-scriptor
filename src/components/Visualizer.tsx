...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Bolt: Pre-calculate gradients outside animation loop
    // Uint8Array strictly bounds values to 0-255
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Bolt: Stop rendering when bars exceed canvas width
  ...
  if (!isPlaying) {
    // Bolt: Use deterministic bar heights instead of Math.random() to fix Next.js hydration mismatch and purity linting error
