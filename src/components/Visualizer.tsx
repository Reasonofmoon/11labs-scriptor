...
export const Visualizer = React.memo<VisualizerProps>(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache array for pre-calculated CanvasGradient objects
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Fast path: stop rendering if off-screen
        ...
        if (!gradientCache[dataValue] && dataValue > 0) {
