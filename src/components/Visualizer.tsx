...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate 256 possible gradients outside the render loop
    // to dramatically reduce Garbage Collection pressure.
    // Uint8Array values are strictly 0-255.
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle zero-height edge case to avoid DOMException
  ...
  if (!isPlaying) {
    // ⚡ Bolt Optimization: Use a static deterministic array instead of Math.random()
    // This fixes Next.js hydration mismatches between server and client renders.
    const idleHeights = [14, 20, 12, 18, 22, 15, 19, 13];
    return (
