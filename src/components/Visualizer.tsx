...
// Pre-defined static heights to replace impure Math.random() in idle state
const IDLE_BAR_HEIGHTS = [12, 18, 10, 24, 15, 20, 14, 22];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradients to avoid expensive operations inside the animation loop
    // dataArray values are strictly between 0 and 255
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Stop drawing if we're off the canvas
