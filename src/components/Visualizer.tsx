...
// Deterministic constant array to avoid Next.js hydration mismatches
// and `react-hooks/purity` lint errors caused by `Math.random()` during render.
const IDLE_BAR_HEIGHTS = [12, 20, 16, 24, 14, 22, 18, 10];
...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Web Audio API frequency data is strictly bounded (0-255 Uint8Array).
    // Pre-calculating exactly 256 gradients outside the render loop
    // avoids creating objects per frequency bin on every requestAnimationFrame,
    // severely reducing Garbage Collection (GC) pressure.
    const gradientCache = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      const gradient = ctx.createLinearGradient(0, Math.max(1, canvas.height - barHeight), 0, Math.max(1, canvas.height));
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Early Break
        // Short-circuit break condition to avoid unnecessary calculation and rendering of off-screen elements.
