...
// Pre-calculate random-looking heights for the idle state to avoid Next.js hydration mismatch
const IDLE_HEIGHTS = [12.5, 18.2, 9.4, 21.8, 15.1, 10.7, 23.5, 14.2];
...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate CanvasGradient objects to prevent high GC pressure.
    // Web Audio API populates dataArray with values 0-255. We cache exactly 256 gradients.
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = Math.max(1, (i / 255) * canvas.height); // Handle zero height
