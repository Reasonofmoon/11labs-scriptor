...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate 256 possible gradients based on Uint8Array values (0-255)
    // This avoids creating new CanvasGradient objects in every frame loop,
    // significantly reducing Garbage Collection pressure.
    const preCalculatedGradients: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const h = (i / 255) * canvas.height;
      const gradientHeight = Math.max(1, h); // Handle zero-height edge case to avoid finite coordinate DOMException
      const gradient = ctx.createLinearGradient(0, canvas.height - gradientHeight, 0, canvas.height);
      gradient.addColorStop(0, primaryColor);
      gradient.addColorStop(1, secondaryColor);
      preCalculatedGradients[i] = gradient;
    }
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Optimization: Stop rendering off-screen bars

        const barHeight = (dataArray[i] / 255) * canvas.height;
        ctx.fillStyle = preCalculatedGradients[dataArray[i]];
  ...
  // Use a deterministic array of static heights to avoid React hydration mismatches in Next.js
  const staticHeights = [12, 22, 14, 18, 10, 20, 16, 24];
