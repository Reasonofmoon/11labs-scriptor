...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // This avoids recreating LinearGradient objects on every animation frame loop
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle edge case where gradient coords might be exactly the same
      const startY = Math.max(0, canvas.height - barHeight);
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Bolt: Early termination if we've rendered past the canvas width
  ...
  // Bolt: Use deterministic heights to prevent hydration mismatches and unnecessary re-evaluations
  const IDLE_BAR_HEIGHTS = [12, 18, 14, 20, 10, 16, 22, 14];
