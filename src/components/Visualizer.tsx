...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Bolt Performance: Pre-calculate gradient cache to avoid object allocation in render loop
    const gradientCache = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle 0 height edge case for createLinearGradient coordinates
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Bolt Performance: Early break if we're drawing off-screen
  ...
  if (!isPlaying) {
    // Bolt Performance: Use deterministic heights instead of Math.random() to prevent hydration errors
    const idleHeights = [12, 18, 14, 20, 10, 22, 16, 12];
    return (
