...
const IDLE_BAR_HEIGHTS = [12, 18, 14, 20, 10, 16, 22, 15];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Cache gradients to avoid creating them every frame for every bar
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let val = 0; val < 256; val++) {
      const barHeight = (val / 255) * canvas.height;
      // Handle the edge case of barHeight being 0 to avoid errors in createLinearGradient
      const safeBarHeight = Math.max(0.1, barHeight);
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // Optimize: Stop drawing if we've exceeded the canvas width
