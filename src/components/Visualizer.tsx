...
// ⚡ Bolt: Static array to prevent hydration mismatch and remove impure Math.random() from render
const IDLE_BAR_HEIGHTS = [12, 18, 10, 22, 14, 20, 11, 16];
...
export const Visualizer: React.FC<VisualizerProps> = React.memo(({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Pre-calculate gradients outside the render loop to reduce object creation overhead
    const gradientCache: CanvasGradient[] = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Provide a fallback of 1 for height to prevent InvalidStateError on 0 height gradients
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt: Stop drawing if we're off-screen
