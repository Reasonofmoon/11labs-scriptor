...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate gradients for all possible values (0-255)
    // This avoids creating 100+ objects per frame inside the render loop
    const gradientCache = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Prevent DOMException: coordinates not finite by enforcing a min height
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Optimization: Stop rendering if off-screen
  ...
  if (!isPlaying) {
    // Deterministic heights to avoid Next.js hydration mismatches from Math.random()
    const heights = [12, 20, 15, 24, 18, 10, 22, 14];
    return (
