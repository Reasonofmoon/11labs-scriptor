...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate gradients since byte frequency data is exactly 0-255.
    // This avoids recreating up to 1024 CanvasGradient objects per frame (60fps = ~61k objects/sec).
  ...
  if (!isPlaying) {
    // ⚡ Bolt Optimization: Replace impure Math.random() with deterministic heights
    // This fixes React hydration mismatches between server and client rendering.
