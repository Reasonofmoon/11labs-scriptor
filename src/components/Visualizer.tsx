...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // Pre-calculate and cache Canvas gradients to avoid high GC pressure in requestAnimationFrame
    const cachedGradients = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      const safeBarHeight = Math.max(1, barHeight); // Handle zero-height edge case
      const gradient = ctx.createLinearGradient(0, canvas.height - safeBarHeight, 0, canvas.height);
      gradient.addColorStop(0, primaryColor);
      gradient.addColorStop(1, secondaryColor);
      cachedGradients[i] = gradient;
    }
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        if (x > canvas.width) break; // Short-circuit to avoid off-screen rendering

        const value = dataArray[i];
        const barHeight = (value / 255) * canvas.height;

        ctx.fillStyle = cachedGradients[value];
  ...
  if (!isPlaying) {
    const idleHeights = [12, 20, 16, 24, 14, 22, 18, 10]; // Deterministic static array to prevent hydration mismatch
    return (
