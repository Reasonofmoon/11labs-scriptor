...
const VisualizerComponent: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate the gradients based on 0-255 frequency data range
    // Web Audio frequency data is strictly Uint8 (0-255).
    // Pre-computing these saves ~200-500 gradient creations per frame!
    const gradientCache = new Array(256);
    for (let i = 0; i < 256; i++) {
      const barHeight = (i / 255) * canvas.height;
      // Handle the edge case of 0 height creating an invalid gradient
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        // ⚡ Bolt Optimization: Break early if we've rendered past canvas width
  ...
  // ⚡ Bolt Fix: Use deterministic heights for idle state to prevent hydration mismatches and impure function warnings
...
// ⚡ Bolt Optimization: Memoize the Visualizer to prevent re-renders when parent state changes
