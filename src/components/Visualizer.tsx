...
export const Visualizer: React.FC<VisualizerProps> = ({ isPlaying, mode, analyser }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate gradients for all 256 possible byte values
    // This removes massive object allocation & GC pressure from the 60fps render loop
  ...
  if (!isPlaying) {
    // Pre-calculated deterministic heights to avoid Math.random() purity/hydration issues
    const idleHeights = [14, 22, 10, 18, 12, 20, 16, 11];
    return (
